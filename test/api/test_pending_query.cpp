...
TEST_CASE("ART position scans release the rollback lock between output batches", "[api]") {
	...
	REQUIRE_NO_FAIL(query_con.Query("INSERT INTO position_rollback "
	                                "SELECT i, CASE WHEN i < 8192 THEN 7 ELSE 8 END, i * 3 "
	...
	REQUIRE_NO_FAIL(*plan);
	REQUIRE(StringUtil::Contains(plan->ToString(), "Type: Index Scan"));
	REQUIRE_NO_FAIL(query_con.Query("BEGIN TRANSACTION"));
	auto &table = Catalog::GetEntry<TableCatalogEntry>(*query_con.context, QualifiedName("position_rollback"))
	                  .Cast<DuckTableEntry>();
	auto table_info = table.GetStorage().GetDataTableInfo();
	REQUIRE_NO_FAIL(query_con.Query("COMMIT"));

	REQUIRE_NO_FAIL(query_con.Query("SET debug_force_commit_failure = true"));
	REQUIRE_NO_FAIL(commit_con.Query("BEGIN TRANSACTION"));
	REQUIRE_NO_FAIL(commit_con.Query("INSERT INTO position_rollback VALUES (10000, 7, 30000)"));
	...
	auto rollback_lock = table_info->TryGetExclusivePositionScanLock();
	REQUIRE(rollback_lock);
	rollback_lock.reset();

	// Rollback truncates the final planned position before the remaining batches resume.
	...
	REQUIRE(result_count == 8192);
	REQUIRE(payload_sum == 100651008);
