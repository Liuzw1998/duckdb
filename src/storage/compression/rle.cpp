...
CompressionFunction GetRLEFunction(PhysicalType data_type) {
	CompressionFunction result(CompressionType::COMPRESSION_RLE, data_type, RLEInitAnalyze<T>, RLEAnalyze<T>,
	                           ...
	                           RLEFilter<T>);
	// LIST selection scans the complete nested column, so the scalar RLE threshold does not apply.
	if (data_type != PhysicalType::LIST) {
		result.position_scan_threshold = 1;
	}
	return result;
}
