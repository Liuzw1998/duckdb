...
//===--------------------------------------------------------------------===//
bool ART::FullScan(ARTRowIdCollection &row_ids) const {
	if (!tree.HasMetadata()) {
		return true;
	}
	Iterator it(*this);
...
bool ART::SearchEqual(const ARTKey &key, ARTRowIdCollection &row_ids) const {
...
bool ART::SearchGreater(const ARTKey &key, bool equal, ARTRowIdCollection &row_ids) const {
	if (!tree.HasMetadata()) {
		return true;
	}
...
bool ART::SearchLess(const ARTKey &upper_bound, bool equal, ARTRowIdCollection &row_ids) const {
	if (!tree.HasMetadata()) {
		return true;
	}
...
bool ART::SearchCloseRange(const ARTKey &lower_bound, const ARTKey &upper_bound, bool left_equal, bool right_equal,
                           ARTRowIdCollection &row_ids) const {
	if (!tree.HasMetadata()) {
		return true;
	}
...
bool ART::Scan(IndexScanState &state, ARTRowIdCollection &row_ids) const {
